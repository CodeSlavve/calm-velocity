          ...
          >
            ...
            <div className="flex-1 overflow-y-auto p-5 space-y-6">
              ...
              <div className="space-y-3 pt-2">
                <h4 className="text-xs font-bold text-slate-400 dark:text-slate-500 uppercase tracking-wider font-mono flex items-center gap-1.5">
                  <Key className="w-3.5 h-3.5 text-indigo-500" />
                  <span>3. Gemini API Key Setup</span>
                </h4>
                
                <div className="bg-slate-50 dark:bg-slate-950/30 border border-slate-150 dark:border-slate-850 p-4 rounded-2xl text-xs space-y-3 leading-relaxed text-slate-600 dark:text-slate-300">
                  <div className="flex items-center justify-between">
                    <span className="font-bold text-slate-700 dark:text-slate-200">Status:</span>
                    {geminiApiKey ? (
                      <span className="px-2 py-0.5 rounded-full text-[10px] font-bold bg-emerald-100 dark:bg-emerald-950/40 text-emerald-700 dark:text-emerald-400 border border-emerald-200 dark:border-emerald-900/30 flex items-center gap-1">
                        <Check className="w-3 h-3" />
                        Active Key
                      </span>
                    ) : (
                      <span className="px-2 py-0.5 rounded-full text-[10px] font-bold bg-rose-100 dark:bg-rose-950/40 text-rose-700 dark:text-rose-400 border border-rose-200 dark:border-rose-900/30">
                        AI Locked (No Key)
                      </span>
                    )}
                  </div>
                  ...
                  <div className="space-y-1.5">
                    ...
                    <div className="relative flex items-center">
                      <input
                        ...
                        placeholder="AIzaSy..."
                        className="w-full text-xs px-3 py-2 bg-white dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-xl focus:outline-none focus:ring-1 focus:ring-indigo-500 font-mono text-slate-700 dark:text-slate-200 pr-10"
                        id="gemini-api-key-input"
                      />
                      <button
                        type="button"
                        onClick={() => setShowKey(!showKey)}
                        className="absolute right-2.5 text-slate-400 hover:text-slate-600 dark:hover:text-slate-300 cursor-pointer"
                  ...
                  <div className="flex gap-2">
                    <button
                      onClick={() => {
                        ...
                        showFeedback("success", apiKeyInput.trim() ? "Gemini API Key saved successfully!" : "Gemini API Key removed.");
                      }}
                      className="flex-1 py-2 bg-indigo-600 hover:bg-indigo-700 text-white font-bold text-xs rounded-xl transition-all cursor-pointer shadow-sm shadow-indigo-100 dark:shadow-none"
                    ...
                    {geminiApiKey && (
                      <button
                        onClick={() => {
                          setApiKeyInput("");
                          onUpdateApiKey("");
                          showFeedback("success", "Gemini API Key cleared.");
                        }}
                        className="py-2 px-3 border border-slate-200 dark:border-slate-800 hover:bg-slate-100 dark:hover:bg-slate-900 text-slate-500 dark:text-slate-400 font-bold text-xs rounded-xl transition-all cursor-pointer"
                  ...
                  <div className="border-t border-slate-150 dark:border-slate-850 pt-2 space-y-1.5">
                    <p className="font-semibold text-slate-700 dark:text-slate-200 text-[11px]">
                      How to login and get your API Key:
                    </p>
                    <ol className="list-decimal list-inside space-y-1 text-[11px] text-slate-500 dark:text-slate-400 font-medium">
                      <li>
                        Click the <span className="font-bold">Get Gemini API Key</span> link below to navigate to Google AI Studio.
                      </li>
                      <li>
                        Sign in with any standard Google/Gmail account.
                      </li>
                      <li>
                        Click the blue <span className="font-bold">"Get API key"</span> button on the top left of their dashboard.
                      </li>
                      <li>
                        Click <span className="font-bold">"Create API key"</span>, copy it, and paste it here!
                      </li>
                    </ol>
                    ...
                    <a
                      ...
                      className="flex items-center justify-center gap-1.5 w-full py-2 bg-slate-100 hover:bg-slate-200 dark:bg-slate-900 dark:hover:bg-slate-800 text-slate-700 dark:text-slate-200 font-extrabold text-[11px] rounded-xl transition-all cursor-pointer mt-1"
                      id="get-gemini-key-btn"
                    >
                      <span>Get Gemini API Key (External)</span>
